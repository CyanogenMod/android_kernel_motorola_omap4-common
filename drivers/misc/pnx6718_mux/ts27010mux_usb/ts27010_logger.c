...
#define MUX_LOG_FILE "/data/mux_usb.log"
#define MUX_OLD_LOG_FILE "/data/mux_usb_old.log"
...
static struct mux_logger_list_t *ts27010_mux_usb_logger_savedata(
	u8 *buf, int len, int type)
{
	struct mux_logger_list_t *mux_list = NULL;
	u8 offset = 0;

	mux_list = (struct mux_logger_list_t *)kmalloc(
		sizeof(struct mux_logger_list_t), GFP_KERNEL);
	if (!mux_list) {
	...
	if (buf[0] == TS0710_BASIC_FLAG)
		...
		offset = 0;
	mux_list->body = (u8 *)kmalloc(len + offset, GFP_KERNEL);
	if (!(mux_list->body)) {
		kfree(mux_list);
		mux_print(MSG_ERROR, "can not malloc (%d) memory!\n", len);
		return NULL;
...
int ts27010_mux_usb_logger_logdata(struct ts27010_mux_logger *mux_logger,
	u8 *buf, int len, int type)
{
	int ret = -ENOMEM;
	...
	mux_list = ts27010_mux_usb_logger_savedata(buf, len, type);
	if (mux_list) {
...
struct ts27010_mux_logger *ts27010_alloc_mux_usb_logger(void)
{
	...
	mux_logger = (struct ts27010_mux_logger *)kmalloc(
	...
	*/
	...
	mux_logger->m_archiver_tsk = kthread_create(
		&mux_logger_archive_thread, mux_logger, "muxusb_logger");
	if (IS_ERR(mux_logger->m_archiver_tsk)) {
		mux_print(MSG_ERROR, "create thread failed\n");
		goto out;
	}
