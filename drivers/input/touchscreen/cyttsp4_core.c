...
static int _cyttsp4_hndshk_enable(struct cyttsp4 *ts)
{
	...
	cmd[1] = 0x03;	/* synchronous level handshake */
	...
	if (tries >= 10000) {
		pr_err("%s: timed out wait for interrupt--%s\n",
		__func__, "trying read anyway");
	...
	if (tries >= 10000) {
		...
		if (retval < 0) {
			pr_err("%s: fail read cmd status"
				" r=%d\n", __func__, retval);
			goto _cyttsp4_write_config_block_exit;
		} else if ((response[0] & CY_CMD_RDY_BIT) != 0x00) {
			pr_err("%s: cmd ready bit not set\n", __func__);
			retval = -EINVAL;
			goto _cyttsp4_write_config_block_exit;
		}
	}
	...
	if (tries >= 10000) {
		...
		if (retval < 0) {
			pr_err("%s: fail read cmd status"
				" r=%d\n", __func__, retval);
			goto _cyttsp4_read_config_block_exit;
		} else if ((pdata[0] & CY_CMD_RDY_BIT) != 0x00) {
			pr_err("%s: cmd ready bit not set\n", __func__);
			retval = -EINVAL;
			goto _cyttsp4_read_config_block_exit;
		}
...
int cyttsp4_resume(void *handle)
{
	...
	retval = ts->platform_data->hw_recov(1);
	if (retval < 0) {
		pr_err("%s: fail irq_wakeup function r=%d\n",
			__func__, retval);
	}

	enable_irq(ts->irq);
	cyttsp4_dbg(ts, CY_DBG_LVL_3, "%s: Driver IRQ now enabled\n",
		__func__);
	ts->irq_enabled = true;
...
int cyttsp4_suspend(void *handle)
{
	...
	cyttsp4_dbg(ts, CY_DBG_LVL_3, "%s: Driver IRQ now disabled\n",
		__func__);
...
_cyttsp4_startup_wait_bl_irq:
	...
	if (retval == 0) {
		pr_err("%s: timeout waiting for application interrupt\n",
			__func__);
		/* Keep going to workaround missed edge interrupts */
	}
	...
	if (!wrote_sysinfo_regs) {
		...
		if (mddata_updated || wrote_settings) {
			pr_err("%s: IC settings already updated\n",
				__func__);
...
_cyttsp4_startup_set_sysinfo_done:
	...
	if (retval < 0) {
		...
		if (wrote_settings) { /* Don't check mddata_updated */
...
static irqreturn_t cyttsp4_irq(int irq, void *handle)
{
	...
	case CY_BL_STATE:
		...
		udelay(2000);	/* Workaround extra level interrupts */
		break;
	case CY_SYSINFO_STATE:
		complete(&ts->si_int_running);
		udelay(200);	/* Workaround extra level interrupts */
		break;
	case CY_LDR_STATE:
