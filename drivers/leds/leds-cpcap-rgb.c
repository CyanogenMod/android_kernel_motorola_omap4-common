...
#define CPCAP_RGB_LED_RED	0x01
#define CPCAP_RGB_LED_GREEN	0x02
#define CPCAP_RGB_LED_BLUE	0x04
#define CPCAP_RGB_LED_BLINK	0x80

#define CPCAP_RGB_LED_REG_IND_BLINK_ON   0x0400
...
static void cpcap_rgb_led_set_regulator(
	struct cpcap_rgb_led_data *cpcap_rgb_data,
	bool enable, bool blinking)
{
	static bool cpcap_rgb_reg_enabled;

	if (cpcap_rgb_data->regulator) {
		if (enable && !cpcap_rgb_reg_enabled) {
			if (!blinking) {
				regulator_enable(cpcap_rgb_data->regulator);
				cpcap_rgb_reg_enabled = true;
			}
			return;
		}
		...
		if (!enable && cpcap_rgb_reg_enabled) {
			...
			cpcap_rgb_reg_enabled = false;
			return;
		}
	}
}

static void cpcap_rgb_led_set_blink(struct cpcap_rgb_led_data *cpcap_rgb_data,
				bool enable)
{
	...
	if (enable) {
		if (!(cpcap_rgb_data->regulator_state & CPCAP_RGB_LED_BLINK)) {
			cpcap_uc_start(cpcap_rgb_data->cpcap, CPCAP_MACRO_6);
			if (debug)
				pr_info("%s: cpcap_uc_start(CPCAP_MACRO_6)\n",
					__func__);

			/* Wait for the macro to start. */
			cpcap_regacc_read(cpcap_rgb_data->cpcap,
					CPCAP_REG_REDC, &led_reg);
			i = 21;
			while ((!(led_reg & CPCAP_RGB_LED_REG_IND_BLINK_ON)) &&
				(--i > 0)) {
				cpcap_uc_start(cpcap_rgb_data->cpcap,
					CPCAP_MACRO_6);
				msleep(10);
				cpcap_regacc_read(cpcap_rgb_data->cpcap,
						CPCAP_REG_REDC, &led_reg);
			...
			if (i == 0)
				printk(KERN_ERR "%s: Unable to sync CPCAP"
					" blink on macro.\n", __func__);

			if (debug)
				pr_info("%s: Blink macro started.\n", __func__);

			/*
			 * Shutdown the regulator since the macro handles the
			 * regulator.
			 */
			cpcap_rgb_led_set_regulator(cpcap_rgb_data, false,
						true);
			cpcap_rgb_data->regulator_state |= CPCAP_RGB_LED_BLINK;
		}
	} else {
		if (cpcap_rgb_data->regulator_state & CPCAP_RGB_LED_BLINK) {
			cpcap_uc_stop(cpcap_rgb_data->cpcap, CPCAP_MACRO_6);

			if (debug)
				pr_info("%s: cpcap_uc_stop(CPCAP_MACRO_6)\n",
					__func__);

			/* Wait for the macro to be stopped before moving on. */
			cpcap_regacc_read(cpcap_rgb_data->cpcap, CPCAP_REG_REDC,
					&led_reg);
			i = 21;
			while ((led_reg & CPCAP_RGB_LED_REG_IND_BLINK_ON) &&
				(--i > 0)) {
				cpcap_uc_stop(cpcap_rgb_data->cpcap,
					CPCAP_MACRO_6);
				msleep(10);
				cpcap_regacc_read(cpcap_rgb_data->cpcap,
						CPCAP_REG_REDC, &led_reg);
			...
			if (i == 0)
				printk(KERN_ERR "%s: Unable to sync CPCAP blink"
					" off macro.\n", __func__);

			if (debug)
				pr_info("%s: Blink macro stopped.\n", __func__);

			/* If any LED's are on re-enable the regulator. */
			if (cpcap_rgb_data->regulator_state&(CPCAP_RGB_LED_RED |
							CPCAP_RGB_LED_GREEN |
							CPCAP_RGB_LED_BLUE))
				cpcap_rgb_led_set_regulator(cpcap_rgb_data,
							true, false);
...
static void cpcap_rgb_led_set_brightness(
	struct cpcap_rgb_led_data *cpcap_rgb_data, int color,
	enum led_brightness value)
{
	...
	if (value == LED_OFF) {
		...
		brightness = rgb_led_config_data.low_value;
	else if (value <= CPCAP_RGB_LOW_MED_LIMIT)
		brightness = rgb_led_config_data.low_med_value;
	else if (value <= CPCAP_RGB_MEDIUM_LIMIT)
		brightness = rgb_led_config_data.med_value;
	else if (value <= CPCAP_RGB_MED_HIGH_LIMIT)
		brightness = rgb_led_config_data.med_high_value;
	else
		brightness = rgb_led_config_data.high_value;
	...
	if (value > LED_OFF) {
		if (!(cpcap_rgb_data->regulator_state & color)) {
			cpcap_rgb_led_set_regulator(cpcap_rgb_data, true,
				((cpcap_rgb_data->regulator_state &
					CPCAP_RGB_LED_BLINK)
					== CPCAP_RGB_LED_BLINK));
		...
		if (cpcap_rgb_data->regulator_state == 0)
			cpcap_rgb_led_set_blink(cpcap_rgb_data, false);
	}
	      ...
	      const char *buf, size_t count)
{
	...
	if (led_blink > LED_OFF)
		cpcap_rgb_led_set_blink(blink_led_data, true);
	else
		cpcap_rgb_led_set_blink(blink_led_data, false);
