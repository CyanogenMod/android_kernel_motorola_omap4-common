...
static int max9635_write_reg(struct max9635_data *als_data, u8 * buf, int len)
{
	...
	do {
		...
		if (err != 1)
			msleep(MAX9635_I2C_RETRY_DELAY);
...
static int max9635_read_adj_als(struct max9635_data *als_data)
{
	...
	lux = lux * als_data->als_pdata->lens_coeff;
