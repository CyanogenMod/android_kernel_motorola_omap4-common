#ifndef __PANEL_MAPPHONE_D2L_H
#define __PANEL_MAPPHONE_D2L_H
...
typedef struct {
	...
	u8  Data_buf[32];
	u16 cksum;
} DSI_long_write_packet;

typedef struct {
	u8  data_type;
	u16 Reg_address;
	u8  ECC;
} DSI_short_read_packet;

typedef struct {
	u8  data_type;
	u16 WC;
	u8  ECC;
	u8  Data_buf[32]; /* note: append 16 bit checksum */
} DSI_long_read_response_packet;
...
/* DSI DCS commands */
/* TODO move these command definitions to common header */
