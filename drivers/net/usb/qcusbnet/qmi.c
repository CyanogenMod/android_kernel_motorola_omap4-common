...
void *qmictl_setdataformat(u8 tid, size_t *size)
{
	struct setdataformat_req *req = kmalloc(sizeof(*req), GFP_KERNEL);
	if (!req)
		return NULL;
	req->req = 0x00;
	req->tid = tid;
	req->msgid = 0x26;
	req->tlvsize = 0x0009;
	req->dataformattype = 0x01;
	req->len = 0x0001;
	req->data_format = 0x00;
	req->llp_protocol_type = 0x10;
	req->len1 = 0x0002;
	req->link_prot = 0x0002;
	*size = sizeof(*req);
	return req;
}
...
int qmictl_setdataformat_resp(void *buf, u16 size)
{
	int result;
	u8 offset = sizeof(struct qmux) + 2;
