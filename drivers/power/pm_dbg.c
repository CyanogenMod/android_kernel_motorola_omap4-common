...
static struct pm_dbg_currmeas_data currmeas_data, currmeas_active_data;
...
static void data_update(struct pm_dbg_data *data, enum update_mode mode)
{
	...
	case PURE_ACTIVE_START:
		currmeas_active_data.time_start = data_get_time_ms();

		data_read_regs(data, &pm_reg);

		currmeas_active_data.sample_start = pm_reg.sample;
		currmeas_active_data.acc_start = pm_reg.accumulator;

		currmeas_active_data.acc_active = 0;
		currmeas_active_data.sample_active = 0;

		currmeas_active_data.time_susp_rpt = 0;
		currmeas_active_data.time_res_rpt = 0;
		currmeas_active_data.avg_curr_susp = 0;
		currmeas_active_data.min_curr_susp = 0;

		currmeas_active_data.acc_susp = 0;

		break;

	case PURE_ACTIVE_STOP:
		currmeas_active_data.time_stop = data_get_time_ms();

		data_read_regs(data, &pm_reg);
		if (pm_reg.sample < currmeas_active_data.sample_start) {
			acc = pm_reg.accumulator -
				(pm_reg.sample * pm_reg.offset);
		} else {
			acc = (pm_reg.accumulator -
				currmeas_active_data.acc_start) -
				((pm_reg.sample -
				currmeas_active_data.sample_start)
				* pm_reg.offset);
		}
		currmeas_active_data.acc_start = pm_reg.accumulator;

		currmeas_active_data.acc_active += acc;
		currmeas_active_data.sample_active += (pm_reg.sample -
			currmeas_active_data.sample_start);

		currmeas_active_data.sample_start = pm_reg.sample;

		test = data_get_avg_curr_ua(data,
				currmeas_active_data.acc_active,
				currmeas_active_data.sample_active);
		printk(KERN_INFO
			"PM_DBG cd-active-uc avg[%d]\n", test);
				 ...
				 pm_message_t state)
{
	...
	if (data->active_meas_enable != 0)
		data_update(data, PURE_ACTIVE_STOP);
	if (data->suspend_meas_enable != 0)
...
static int pm_dbg_resume(struct platform_device *pdev)
{
	...
	if (data->active_meas_enable != 0)
		data_update(data, PURE_ACTIVE_START);
