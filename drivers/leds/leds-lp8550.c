...
static void lp8550_brightness_write(struct lp8550_data *led_data)
{
	...
	if (!led_data->suspended) {
		switch (led_data->led_pdata->dev_ctrl_config) {
		case LP8550_BRT_MODE_BRIGHT:
		...
		case LP8550_BRT_MODE_PWM:
			//scale the brightness to prevent more than 19mA per LED
			scaling_factor = led_data->led_pdata->scaling_factor;
			if (lp8550_write_reg(led_data, LP8550_EEPROM_A0,
				 (brightness * scaling_factor) / 1000))
				pr_err("%s:Failed to set brightness\n",
				__func__);
			break;
		...
		if (!atomic_cmpxchg(&led_data->enabled, 0, 1)) {
			/* This is where any resume code that MUST be run after
			 * the framework is ready should be placed.  It will
			 * only be run once after each resume. */
			if (led_data->led_pdata->dev_ctrl_config ==
				LP8550_BRT_MODE_PWM) {
				/* We cannot enable the PWM until the framework
				 * is ready, or else there might be old data in
				 * the framebuffer and we might get flickers on
				 * the backlight */
				if (lp8550_write_reg(led_data,
					LP8550_DEVICE_CTRL,
					LP8550_BRT_MODE_PWM | 0x01)) {
		...
		pr_warn("%s: LED brightness write while suspended, ignoring\n",
			__func__);
	}
}
...
static void lp8550_early_suspend(struct early_suspend *h)
{
	...
	switch (led_data->led_pdata->dev_ctrl_config) {
	case LP8550_BRT_MODE_PWM:
		if (lp8550_write_reg(led_data, LP8550_EEPROM_A0, LED_OFF)) {
			pr_err("%s:writing failed while setting brightness\n",
				__func__);
		}
		/* fallthrough */

	case LP8550_BRT_MODE_BRIGHT:
		if (lp8550_write_reg(led_data, LP8550_BRT_MODE_BRIGHT,
				LED_OFF)) {
			pr_err("%s:writing failed while setting brightness\n",
				__func__);
		}
...
static void lp8550_late_resume(struct early_suspend *h)
{
	...
	/* We will always start out in BRIGHTNESS mode with brightness 0.
	 * If PWM mode is to be used, it will be enabled the first time
	 * that the brightness is written by the framework. */
	if (lp8550_write_reg(led_data, LP8550_BRT_MODE_BRIGHT,
			LED_OFF)) {
		pr_err("%s:writing failed while setting brightness\n",
			__func__);
	...
	if (lp8550_write_reg(led_data, LP8550_DEVICE_CTRL,
			LP8550_BRT_MODE_BRIGHT | 0x01)) {
		pr_err("%s:writing failed while setting brightness\n",
			__func__);
	}

	/* restore eeprom values in case they have been reset */
	for (i = 0; i < led_data->led_pdata->eeprom_tbl_sz; i++) {
		reg_addr = LP8550_EEPROM_A0 + i;
		value = led_data->led_pdata->eeprom_table[i].eeprom_data;
		if (lp8550_write_reg(led_data, reg_addr, value))
