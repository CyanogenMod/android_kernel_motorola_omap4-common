...
defer_bh(struct usbnet *dev, struct sk_buff *skb, struct sk_buff_head *list)
{
	...
	if (dev->done.qlen == 1 && !test_bit(EVENT_RX_MEMORY, &dev->flags))
...
void axusbnet_defer_kevent (struct usbnet *dev, int work)
{
	...
	if (!schedule_work (&dev->kevent)) {
		/* since can't schedule workqueue, it is often because
		 of low memory, just print log 5 times. Don't print
		 forevery*/
		if (count++ > 5)
			return;
