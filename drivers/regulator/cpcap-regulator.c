...
#define REGULATOR_STATE_LEN 512
...
static struct {
	...
	[CPCAP_VSIM]     = {CPCAP_REG_VSIMC,
			    CPCAP_REG_ASSIGN3,
			    /*
			    * VSIM and VSIMCARD are on the same control bit,
			    * do not allow secondary control.
			    */
			    0,
			    0x0023,
			    0x0008,
...
static int cpcap_regulator_state;
				       ...
				       unsigned *selector)
{
	...
	*selector = i;
	...
	if ((cpcap_regltr_data[regltr_id].volt_trans_time) && (retval == 0)) {
		if (cpcap_regltr_data[regltr_id].volt_trans_time >=
		    (MAX_UDELAY_MS * USEC_PER_MSEC)) {
			mdelay(cpcap_regltr_data[regltr_id].volt_trans_time /
			       USEC_PER_MSEC);
...
static int cpcap_regulator_enable(struct regulator_dev *rdev)
{
	...
	regnr = cpcap_regltr_data[regltr_id].reg;
	cpcap_regulator_state |= (1 << regltr_id);
	...
	if ((cpcap_regltr_data[regltr_id].turn_on_time) && (retval == 0)) {
		if (cpcap_regltr_data[regltr_id].turn_on_time >=
		    (MAX_UDELAY_MS * USEC_PER_MSEC)) {
			mdelay(cpcap_regltr_data[regltr_id].turn_on_time /
			       USEC_PER_MSEC);
...
void cpcap_regulator_print_state(struct seq_file *s, bool log)
{
	static char buf[REGULATOR_STATE_LEN];
	char *pbuf;
	int len;
	int name_len;
	int i = 0;

	memset(buf, 0, REGULATOR_STATE_LEN);
	pbuf = buf;
	len = REGULATOR_STATE_LEN - (pbuf - buf);

	if (len > 13)
		pbuf += snprintf(pbuf, len, "Regulators: ");

	/* get state for each regulator */
	for (i = 0; i < ARRAY_SIZE(regulators); i++) {

		/* igore the regulator which do not have name */
		if (!regulators[i].name)
			continue;

		len = REGULATOR_STATE_LEN - (pbuf - buf);
		name_len = strlen(regulators[i].name);
		if (len > (10 + name_len)) {
			pbuf += snprintf(pbuf, len, "%d:%s[%s]," , i,
				regulators[i].name,
				(char *)((cpcap_regulator_state &
				(1<<regulators[i].id)) > 0 ? "ON" : "OFF"));
		}
...
EXPORT_SYMBOL_GPL(cpcap_regulator_print_state);
...
static int __devinit cpcap_regulator_probe(struct platform_device *pdev)
{
	...
	if (pdev->id == CPCAP_SW5) {
		...
		data->regulator_init->supply_regulator = "sw5";
