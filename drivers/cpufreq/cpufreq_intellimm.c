...
#define DEF_FREQUENCY_UP_THRESHOLD_MULTY	(60)
#define DEF_FREQUENCY_UP_THRESHOLD_ANY_CPU	(60)
...
#define DEF_TWO_PHASE_FREQUENCY			(1400000)
#define DBS_INPUT_EVENT_MIN_FREQ		(1036800)
#define DEF_FREQUENCY_OPTIMAL			(800000)
#define DEF_FREQ_DOWN_STEP			(100000)
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		cur_idle_time = get_cpu_idle_time_us(j, &cur_wall_time, io_busy);
...
static int cpufreq_gov_dbs_up_task(void *data)
{
	...
	while (1) {
		...
		this_dbs_info->prev_cpu_idle = get_cpu_idle_time_us(cpu,
