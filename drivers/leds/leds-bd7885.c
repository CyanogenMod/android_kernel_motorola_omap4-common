...
static int bd7885_reg_read(struct i2c_client *client, u8 reg_addr, u8 *buf)
{
	...
	pr_err("invalid read parameter(s)\n");
	return -EINVAL;
    }
    /* If I2C client doesn't exist */
    if (client == NULL) {
	pr_err("bd7885 null i2c read client\n");
	return -EUNATCH;
    }
	...
	* (i2c_master_send returns the number of bytes transferred) */
	if (retval == BD7885_REG_DATA_LEN) {
		...
		*/
		...
		pr_err("bd7885 write of address %d failed: %d\n",
				value[0], retval);
	}
		...
		u8 reg_addr, u8 reg_value)
{
	...
	pr_err("bd7885_reg_write: length is invalid\n");
	return -EINVAL;
    }

    /* Fail if we weren't able to initialize (yet) */
    if (client == NULL) {
	pr_err("bd7885_reg_write: initialization failed\n");
	return -EINVAL;
    }
	...
	if (retval < 0) {
		pr_err("bd7885_reg_write: write of reg 0x%X failed: %d\n",
				value[0], retval);
		msleep(10);
	}
    } while ((retval < 0) && (i-- >= 0));
	...
	unsigned char level)
{
    ...
    if (ret != 0) {
	pr_err("bd7885_quench_level_set failed\n");
	return -EPERM;
    }
...
static int bd7885_charge_enable_sequence(struct i2c_client *client)
{
	...
	pr_err("bd7885_reg_writes failed\n");
	return -EPERM;
    }
...
int bd7885_charge_disable_sequence(struct i2c_client *client)
{
	...
	pr_err(KERN_ERR "bd7885_reg_writes failed\n");
	return -EPERM;
    }
...
static int bd7885_charge_enable(struct i2c_client *client)
{
	...
	pr_err("%s: bd7885_reg_writes failed\n", __func__);
	return -EPERM;
    }
...
bool bd7885_device_detection(void)
{
	...
	if (ret != 0) {
		pr_err("%s: failed to read ID register\n", __func__);
		return false;
	}
...
void bd7885_device_enable(void)
{
	...
	if (flash_control_mode == FLASH_UNKNOWN) {
		...
		if (flash_control_mode == FLASH_UNKNOWN) {
			...
			pr_debug("%s: flash module not detected\n", __func__);
		} else {
			pr_debug("%s: flash module detected in mode %d\n",
			...
			const struct i2c_device_id *id)
{
	...
	pr_debug("BD7885 driver: bd7885_probe()\n");
	...
	pr_debug("BD7885 chip probe is finished.\n");
...
static int bd7885_remove(struct i2c_client *client)
{
	...
	pr_debug("bd7885_remove() is called!\n");
	kfree(chip);
...
static int bd7885_open(struct inode *inode, struct file *file)
{
  pr_debug(KERN_INFO "%s is called.\n", __func__);
		...
		unsigned int cmd, unsigned long arg)
{
	...
	if (bd7885_reg_read(bd7885_i2c_client,
		...
		pr_err("bd7885_reg_read_filed. reg_addr = %d\n",
					reg_cnt);
		return -EPERM;
	}
     }
...
static void __exit bd7885_exit(void)
{
	...
	pr_debug("BD7885 flash driver: exit\n");
}
