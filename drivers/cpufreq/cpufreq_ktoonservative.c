...
#define DEF_FREQUENCY_UP_THRESHOLD_HOTPLUG	(70)
...
#define DEF_BOOST_HOLD_CYCLES			(9)
...
struct work_struct kthotplug_offline_work;
struct work_struct kthotplug_online_work;
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	if (max_load > dbs_tuners_ins.up_threshold_hotplug) {
		if (num_online_cpus() < 2 && policy->cur != policy->min)
		{
			...
			{
				schedule_work_on(0, &kthotplug_online_work);
	...
	if (max_load < dbs_tuners_ins.down_threshold_hotplug && !dbs_tuners_ins.disable_hotplugging) {
		if (num_online_cpus() > 1)
		{
			...
			{
				schedule_work_on(0, &kthotplug_offline_work);
...
static void kthotplug_offline_work_fn(struct work_struct *work)
{
	int cpu;
...
static void kthotplug_online_work_fn(struct work_struct *work)
{
	int cpu;
...
{
	INIT_WORK(&kthotplug_offline_work, kthotplug_offline_work_fn);
	INIT_WORK(&kthotplug_online_work, kthotplug_online_work_fn);
