/*
 * include/linux/attint48mu_cap_prox.h
 *
...
#ifndef _LINUX_ATTINY28MU_CAP_PROX_H
#define _LINUX_ATTINY28MU_CAP_PROX_H

struct cap_prox_cfg {
	uint8_t	lp_mode;
	uint8_t	lp_mode_active;
	uint8_t	address_ptr;
	uint8_t	reset;
...
struct attiny48mu_cap_prox_platform_data {
	int irq;
	int poll_interval;
	...
	uint8_t key1_ref_drift_thres_l_sleep;
	uint8_t key3_ref_drift_thres_l_sleep;
	uint8_t key1_ref_drift_thres_h_sleep;
	uint8_t key3_ref_drift_thres_h_sleep;
	uint8_t ref_drift_diff_thres_sleep;
...
#endif /* _LINUX_ATTINY28MU_CAP_PROX_H */
